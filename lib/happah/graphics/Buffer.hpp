...
Buffer make_buffer(const std::vector<GLboolean>& points, GLenum usage = GL_STATIC_DRAW);

Buffer make_buffer(const std::vector<GLint>& values, GLenum usage = GL_STATIC_DRAW);
